...
분석할 문서가 굉장히 많다면 문서를 주제별로 분류해 핵심 단어를 살펴보면서 어떤 내용을 담고 있는지 파악해야 합니다. 문서에서 핵심 주제를 찾아 분류하는 작업을 **토픽 모델링(Topic modeling)**이라고 합니다.
...
예시를 들어 토픽 모델링의 개념을 알아보겠습니다. 아래에 3개의 문서가 있습니다. 문서 1은 고양이, 문서 2는 음식, 문서 3은 고양이와 음식 모두와 관련된 내용을 담고 있습니다. 이 문서들을 토픽별로 분류하고, 각 토픽이 어떤 내용을 담고 있는지 알아보려고 합니다.


```{r echo=FALSE, out.width = '400px'}
knitr::include_graphics(here::here("img/topic_modeling_01.png"))
```

*(문서 목록)*
<br>

토픽의 개수를 2개로 정해 모델을 만들었다고 가정하겠습니다. 토픽 모델은 문서(document)와 단어(term)의 관계를 이용해 만듭니다. 토픽 모델을 만들면 문서에 사용된 단어가 두 토픽 중 어디에 등장할 가능성이 더 높은지 알 수 있습니다.

다음 표를 보면 토픽 A는 고양이 관련 단어, 토픽 B는 음식 관련 단어가 등장할 가능성이 높습니다. 이처럼 토픽 모델을 이용하면 문서가 어떤 토픽으로 구성되는지, 각 토픽이 어떤 내용을 담고 있는지 알 수 있습니다.


```{r echo=FALSE, out.width = '200px'}
knitr::include_graphics(here::here("img/topic_modeling_02.png"))
```
...
토픽 모델을 만들면 각 문서가 어떤 토픽에 등장할 확률이 높은지 알 수 있습니다. 이 확률을 이용하면 각 문서를 확률이 높은 토픽으로 분류할 수 있습니다. 다음의 경우 문서 1과 3은 토픽 A로, 문서 2는 토픽 B로 분류할 수 있습니다.

```{r echo=FALSE, out.width = '200px'}
knitr::include_graphics(here::here("img/topic_modeling_03.png"))
```
...
이처럼 토픽 모델을 만들면 문서에 어떤 주제들이 있는지 파악할 수 있습니다. 특히 주제별로 문서를 분류할 수 있기 때문에 다량의 문서를 분석할 때 유용합니다.


> [참고] 앞의 예시의 토픽 모델의 의미를 쉽게 이해하기 위해 만든 것입니다. 실제 토픽 모델을 만들면 하나의 문서가 여러 토픽에 등장할 가능성이 있고, 단어도 모든 토픽에 등장할 가능성이 있기 때문에 확률이 0이 되지는 않습니다.


### LDA 모델 알아보기

**LDA(Latent Dirichlet Allocation, 잠재 디리클레 할당)**는 가장 널리 사용되는 토픽 모델링 알고리즘입니다. LDA는 다음과 같은 가정을 하고 토픽과 단어의 관계를 추정합니다.

#### 토픽은 단어들의 혼합으로 구성된다

토픽에는 여러 단어가 서로 다른 확률로 포함되어 있습니다. '고양이' 토픽과 '음식' 토픽을 예로 들어보겠습니다. 고양이 토픽에는 '털', '모래', 음식 토픽에는 '김밥', '참치'라는 단어가 흔하게 등장할 겁니다. 그런데 '참치'는 고양이가 좋아하는 음식이기 때문에 양쪽 토픽 모두에 등장할 수 있습니다. 다만 고양이와 함께 자주 언급되는 단어이므로 음식 토픽보다는 고양이 토픽에 등장할 확률이 더 높을 겁니다. 즉, 확률은 다르지만 같은 단어가 여러 토픽에 공통적으로 포함되어 있는 것입니다.

#### 문서는 토픽들의 혼합으로 구성된다

문서에는 여러 토픽의 단어가 서로 다른 비율로 포함되어 있습니다. 예를 들어 문서 1에는 토픽 A의 단어가 90%, 토픽 B의 단어가 10% 포함된 반면, 문서 2에는 토픽 A의 단어가 30%, 토픽 B의 단어가 70% 포함될 수 있습니다.

다음 그림에서 왼쪽은 토픽이고 오른쪽은 문서입니다. 그림에서 보듯 문서는 무 자르듯이 어느 한 토픽에만 속하는 게 아니라 모든 토픽에 속할 확률을 어느 정도씩 가지고 있습니다. 다만 문서를 분류할 때는 확률이 더 높은 토픽을 기준으로 합니다. 예를 들어 문서 1은 토픽 A의 단어 확률이 높으니 토픽 A로, 문서 2는 토픽 B의 단어 확률이 높으니 토픽 B로 분류하는 것입니다.

```{r echo=FALSE, out.width = '480px', fig.cap="Latent Dirichlet allocation(LDA)"}
knitr::include_graphics(here::here("img/lda_Blei.png"))
```

- Probabilistic Topic Models: [cs.columbia.edu/~blei/talks/Blei_ICML_2012.pdf](www.cs.columbia.edu/~blei/talks/Blei_ICML_2012.pdf)

<br>

> [참고] LDA 모델이 만들어지는 과정을 자세히 알고 싶다면 아래 글을 읽어보세요.
...
> [참고] 예문을 이용해 토픽 모델을 설명하는 아이디어를 아래 글에서 얻었습니다.

> [wikidocs.net/30708](wikidocs.net/30708)



## 6.2 LDA 모델 만들기

LDA 모델의 개념을 살펴봤으니 기생충 기사 댓글을 이용해 LDA 모델을 만드는 방법을 알아보겠습니다.
...
기생충 기사 댓글을 불러와 기본적인 전처리를 하겠습니다. 앞 장까지와 달리 토픽 모델을 만드는데 적합하도록 몇 가지 전처리 작업을 추가해야합니다.

**중복 문서 제거하기** : 중복된 문서가 있으면 계산량이 늘어나 모델을 만드는 시간이 오래 걸리고, 한 토픽에 내용이 같은 여러 문서가 분류되는 문제가 생깁니다. 이런 문제를 피하기 위해 `dplyr` 패키지의 `distinct()`를 이용해 중복 댓글을 제거하겠습니다. 댓글이 들어있는 `reply`외에 다른 변수들도 모두 보유하도록 `.keep_all = T`를 입력하겠습니다.

**짧은 문서 제거하기**: 토픽 모델은 여러 문서에 공통으로 사용된 단어를 이용해 토픽을 찾는 과정을 거쳐 만듭니다. 짧은 문서는 다른 문서와 공통으로 사용된 단어가 적기 때문에 토픽 모델을 만드는데 적합하지 않습니다. 짧은 문서를 제거하기 위해 `str_count()`로 3 단어 이상 사용된 댓글만 추출하겠습니다.
...
> [참고] 문서를 토픽별로 분류하는 작업을 할 때 `raw_news_comment`를 활용하게 됩니다. 이때 문서 번호를 기준으로 삼기 때문에 `row_number()`로 고유 번호를 부여해 두어야 합니다.

#### 2. 명사 추출하기

문서의 주제는 명사로 결정됩니다. 그래서 토픽 모델을 만들 때는 명사를 추출해 활용하는 경우가 많습니다. `news_comment`에서 명사를 추출하겠습니다.

문서에 같은 단어가 여러 번 사용되면 내용에 관계없이 단순히 사용 빈도 때문에 특정 토픽으로 분류될 가능성이 높아집니다. 이런 문제를 막기 위해 각 댓글에서 중복 사용된 단어를 제거하겠습니다.
...
분석에 사용하는 문서가 영화 기생충에 관한 내용이기 때문에 대부분 '영화', '기생충'과 같은 단어가 공통적으로 들어있습니다. 빈도가 매우 높은 단어가 포함된 상태로 토픽 모델을 만들면 거의 모든 토픽에 같은 단어가 주요 단어로 등장해 토픽의 특징을 파악하기 어렵습니다. 빈도가 높은 단어를 제외하고 모델을 만들면 이런 문제를 피할 수 있습니다. 단어 빈도가 200회 이하인 단어만 추출해 분석에 활용하겠습니다.
...
`count_word`에는 `"들이"`, `"하다"`, `"하게"`와 같이 의미를 알 수 없는 단어가 들어있습니다. 이런 단어들은 텍스트를 해석하는 데 도움이 되지 않으므로 제외해야 합니다. 이처럼 분석에서 제외할 단어를 **불용어(Stop word)**라고 합니다. `count_word`에서 빈도가 높은 단어를 추출해 불용어가 있는지 확인 하겠습니다. 또한 표현은 다르지만 의미가 비슷한 유의어가 있는지 확인하겠습니다.
...
`count_word`에서 불용어를 제거한 다음 `dplyr` 패키지의 `recode()`를 이용해 유의어를 수정하겠습니다. `recode()`는 특정 값을 다른 값으로 수정하는 기능을 합니다.
...
> [참고] `tm`패키지의 `inspect()`를 이용하면 DTM의 내용을 확인할 수 있습니다.
...
```{r, echo=F}
library(ldatuning)
models <- readRDS(here::here("files_etc/lda_models.rds"))
