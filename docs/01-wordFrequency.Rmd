...
include_graphics("https://raw.githubusercontent.com/youngwoos/Doit_textmining/main/Image/01/01_3_7.png")
```
...
.info[`r icon::fa("rocket", color = "#FF7333")` 정규 표현식이란?]
...
`r icon::fa("lightbulb")` 정규 표현식 패턴들(오픈튜토리얼스): [bit.ly/easytext_12](https://bit.ly/easytext_12)
]
...
`r icon::fa("exclamation-triangle", color = "#FF7333")` `unnest_tokens()`에는 tibble 또는 데이터 프레임 구조 입력
...
.info[`r icon::fa("rocket", color = "#FF7333")` macOS에서 그래프에 한글 표현하기]
...
`r icon::fa("lightbulb")` Plots 창 크기에 따라 그래프 모양 달라짐
...
`r icon::fa("lightbulb")` 워드 클라우드 디자인 바꾸기: [lepennec.github.io/ggwordcloud](https://lepennec.github.io/ggwordcloud/)
...
`r icon::fa("lightbulb")` [fonts.google.com](fonts.google.com)
...
`r icon::fa("lightbulb")` RStudio를 실행할 때마다 `showtext` 패키지로 폰트를 설정해야 합니다.

---

.box[

.info[`r icon::fa("rocket", color = "#FF7333")` `ggplot2` 기본 테마 폰트 변경하기]
