...
```{r echo=FALSE, R.options=list(tibble.width = 80, width = 90, pillar.min_chars = Inf)}
top10 <- frequency_wide %>%
  group_by(president = ifelse(log_odds_ratio > 0, "moon", "park")) %>%
  slice_max(abs(log_odds_ratio), n = 10, with_ties = F)
...
<br-back-20>
.scroll-box-20[
```{r echo=FALSE, R.options=list(tibble.width = 80, width = 80, pillar.min_chars = Inf)}
top10 %>%
  arrange(-log_odds_ratio) %>%
  select(word, log_odds_ratio, president)
```
...
```{r echo=F, R.options=list(tibble.width = 80, width = 100, pillar.min_chars = Inf), highlight.output=c(5:14)}
top10 %>%
  arrange(-log_odds_ratio) %>%
  select(word, log_odds_ratio, president)
```
...
```{r echo=F, R.options=list(tibble.width = 80, width = 100, pillar.min_chars = Inf), highlight.output=c(15:24)}
top10 %>%
  arrange(-log_odds_ratio) %>%
  select(word, log_odds_ratio, president)
```
...
```{r echo=FALSE, R.options=list(tibble.width = 80, width = 100, pillar.min_chars = Inf)}
frequency_wide %>%
  filter(rank(odds_ratio) <= 10 | rank(-odds_ratio) <= 10) %>%
  arrange(-odds_ratio)
```
