...
<!-- 문자 길이 무제한 설정 -->
```{r echo=F, R.options=list(tibble.width = Inf, width = 80, pillar.min_chars = Inf)}
top10
```
---
...
```{r echo=F, fig.width=5, fig.height=4, out.width="70%"}
# 주요 단어 추출
top10 <- frequecy %>%
  group_by(president) %>%
  slice_max(tf_idf, n = 10, with_ties = F)
...
```{r eval=FALSE, R.options=list(tibble.width = 50)}
# 데이터 불러오기
