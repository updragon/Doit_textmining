...
# 7. 텍스트 마이닝 프로젝트: 타다 금지법 기사 댓글 분석

'타다 금지법'으로 불리는 '여객자동차 운수사업법 개정안'이 2019년 12월 5일 국회 국토교통위원회 교통법안심사소위를 통과했습니다. 법안에 관광 목적으로 11~15인승 승합차를 빌리는 경우에만 운전자를 알선할 수 있다는 규정이 있기 때문에 타다는 더 이상 서비스를 유지할 수 없는 상황에 놓였습니다. 이를 두고 정부가 혁신 서비스를 막는다는 의견과 택시 업계의 생존을 위해 타당하다는 의견 등 다양한 입장이 있었습니다. 타다 금지법을 다룬 네이버 뉴스 기사에 달린 댓글을 분석하고 사람들이 어떤 생각을 가지고 있는지 알아보겠습니다.


## 7.1 주요 단어 살펴보기

타다 금지법 기사 댓글에 사람들이 가장 많이 언급한 단어가 무엇인지 알아보기 위해 단어 빈도를 분석하겠습니다. 데이터를 불러와 전처리한 뒤 명사를 추출해 빈도를 구하겠습니다. 그런 다음 빈도가 높은 주요 단어를 추출해 그래프를 만들고 내용을 살펴보겠습니다.
...
`"news_comment_tada.csv"`의 `reply`에 '타다 금지법'을 다룬 뉴스 기사에 달린 댓글이 담겨 있습니다. 띄어쓰기가 전혀 없는 텍스트는 `KoNLP` 패키지를 이용해 품사를 추출할 수 없으므로 띄어쓰기가 1개 이상 사용된 댓글만 추출하겠습니다. 원문을 확인하는 용도로 활용하기 위해 html 태그를 제거해 `reply_raw`에 할당해 둔 다음, `reply`를 형태소분석기를 이용하는데 적합하게 전처리하겠습니다.
...
raw_tada <- read_csv(here::here("files/news_comment_tada.csv")) %>%
  mutate(id = row_number())

glimpse(raw_tada, width = 60)
```
...
댓글에서 명사를 추출해 빈도를 구하고, 빈도가 높은 상위 30개 단어를 출력하겠습니다.
...
library(tidytext)
library(KoNLP)
# saveRDS(word_noun, "tada_word_noun.rds")
word_noun <- readRDS(here::here("preprocessing/news_coments/tada/tada_word_noun.rds"))
```
...
출력 결과를 보면 `"들이"`, `"하게"`와 같이 의미를 알 수 없는 단어가 포함되어 있습니다. 불용어 목록을 만들어 `frequency`에서 제외한 다음, 빈도가 높은 상위 20개 단어를 다시 추출하겠습니다.
...
`top20_noun`를 이용해 막대 그래프를 만들겠습니다. 출력 결과를 보면 타다와 택시의 관계를 언급한 댓글이 많기 때문에 `"택시"`, `"기사"`의 빈도가 가장 높습니다. `"혁신"`, `"서비스"`, `"규제"` 등을 보면 법안의 성격을 다룬 댓글이 많다는 것을 알 수 있고, `"정부"`, `"국회의원"`을 보면 법안을 발의한 주체를 다룬 댓글이 많다는 것을 알 수 있습니다.
...
> [참고] `geom_text()`에 삽입한 `comma(n, accuracy = 1)`은 `n`의 세 자릿수 마다 쉼표를 삽입하고 소수점 첫째 자리에서 반올림하는 기능을 합니다. `comma()`는 `scales` 패키지에 포함되어 있으므로 `scales` 패키지를 로드한 다음 사용해야 합니다.


## 7.2 공감, 비공감 댓글 비교하기 - 로그 오즈비

네이버 뉴스 기사 댓글에는 공감, 비공감 버튼이 있습니다. 로그 오즈비를 이용해 공감이 많은 댓글과 비공감이 많은 댓글의 주요 단어를 비교해보겠습니다.
...
댓글을 공감과 비공감으로 분류하는 파생 변수를 만든 다음 공감 여부별 단어 빈도를 구하겠습니다. 그런 다음 데이터를 Wide form으로 변형해 로그 오즈비를 구하고, 각 카테고리에서 상대적으로 중요한 단어를 추출하겠습니다. 마지막으로 그래프를 만들어 내용을 살펴보겠습니다.


#### 1. 공감 여부 변수 만들기

`word_noun`의 `sympathyCount`는 공감 수, `antipathyCount`는 비공감 수를 의미합니다. 변수명을 다루기 쉽게 수정하고, 두 변수의 차이를 나타낸 `diff`를 추가하겠습니다. 그런 다음 `diff`가 `1` 이상이면 `"like"`, `-1` 이하면 `"dislike"`, 그 외에는 `"neutral"`로 분류해 공감 여부를 나타낸 `sympathy`를 추가하겠습니다.
...
`sympathy`별 댓글 개수를 확인해보겠습니다. `word_sympathy`는 한 행이 댓글별 명사이므로 여러 댓글이 중복되어 있습니다. `distinct()`를 이용해 댓글별로 한 행씩만 남긴 후 `sympathy`별 빈도를 구하겠습니다. 출력 결과를 보면 `"neutral"`이 가장 많고, 그 뒤로는 `"like"`, `"dislike"` 순으로 많다는 것을 알 수 있습니다.
...
공감 여부 및 단어별 빈도를 구하겠습니다. 공감, 비공감 댓글을 비교하는 게 분석 목적이므로 `sympathy`가 `"centrism"`인 댓글은 제거하겠습니다. 그런 다음 데이터를 Wide form으로 변형해 로그 오즈비를 구하겠습니다. `log_odds_ratio`는 분모에 `"dislike"`, 분자에 `"like"`의 빈도를 놓고 만들었으므로, 값이 클수록 공감 댓글에서 상대적으로 중요한 단어, 작을수록 비공감 댓글에서 상대적으로 중요한 단어라고 할 수 있습니다.
...
댓글에서 20회 이상 사용된 단어를 대상으로 로그 오즈비가 `0`보다 크면 `"like"`, 그 외에는 `"dislike"`로 분류한 다음, 로그 오즈비가 가장 높은 단어와 가장 낮은 단어를 10개씩 추출하겠습니다.
...
<!-- 댓글에서 20회 이상 사용된 단어를 대상으로, 로그 오즈비가 가장 높은 단어와 가장 낮은 단어를 10개씩 추출하겠습니다. 그런 다음, 로그 오즈비가 0보다 크면 `"like"`, 그 외에는 `"dislike"`를 부여한 파생변수를 만들겠습니다. -->
...
<!--   # 공감 여부 변수 추가 -->
<!--   mutate(sympathy = ifelse(log_odds_ratio > 0, "like", "dislike")) -->
<!-- ``` -->
...
`top10_odds`를 이용해 막대 그래프를 만들겠습니다. 막대 색깔은 색상 코드(Hex Color Code)로 목록을 만들어 지정하겠습니다. 출력된 그래프를 보면 공감 댓글에는 `"조합"`, `"소비자"`, `"동남아"`와 같은 단어를 많이 언급한 반면, 비공감 댓글에는 `"렌트카"`, `"한국"`, `"댓글"` 과 같은 단어를 상대적으로 많이 언급했음을 알 수 있습니다.
...
#### 1. 관심 단어를 언급한 댓글 추출하기

주요 단어를 언급한 댓글 원문을 살펴보겠습니다. `filter(str_detect())`를 이용하면 관심 단어를 언급한 댓글을 추출할 수 있습니다.
...
출력한 텍스트가 길면 관심 단어가 어떤 문장에 사용됐는지 찾기 어렵습니다. 관심 단어만 폰트 스타일을 바꾸어 출력하면 긴 텍스트에서 관심 단어를 쉽게 알아볼 수 있습니다.

#### (1) 텍스트에 스타일 입히기 - `combine_styles()`

`crayon` 패키지의 `combine_styles()`는 텍스트에 스타일을 적용하는 '스타일 함수'를 만들어 줍니다. 텍스트에 스타일 함수를 적용해 `cat()`을 이용하면 스타일이 바뀐 텍스트를 출력할 수 있습니다.
...
> [편집] 스타일 적용 - "폰트를 적용해 출력"

<br>

> [참고] `crayon`패키지는 `dplyr` 패키지를 설치할 때 함께 설치됩니다. `crayon` 패키지를 이용하면 텍스트에 다양한 스타일을 적용할 수 있습니다. 아래 글을 참고하세요.

>  [github.com/r-lib/crayon](https://github.com/r-lib/crayon)


`"조합"`을 언급한 댓글을 추출한 다음 스타일을 적용해 출력해 보겠습니다. `str_detect()`를 이용해 `"조합"`을 언급한 댓글을 추출하겠습니다. 그런 다음 `paste0()`와 `font()`를 이용해 댓글에서 `"조합"`에 스타일을 적용하겠습니다. 마지막으로 `pull()`을 이용해 `reply`의 텍스트만 추출한 다음 `cat()`을 이용해 출력하겠습니다. `cat()`에 입력한 `sep = "\n\n"`는 각 댓글을 줄을 바꾸어 새 행에 출력하는 기능을 합니다. 출력 결과를 보면 관심 단어를 쉽게 파악할 수 있습니다.
...
#### (2) 관심 단어가 사용된 텍스트를 추출해 스타일을 입히는 함수 만들기

앞으로 관심 단어를 언급한 댓글을 추출해 스타일을 적용하는 작업을 반복하므로 함수를 만들어 활용하겠습니다.
...
`find_word()`에 데이터 프레임을 적용하고 `x`에 텍스트가 들어있는 변수, `keyword`에 관심 단어, `n`에 추출할 행 수를 입력하면 스타일이 적용되어 출력됩니다. `n`을 입력하지 않으면 6행을 출력합니다.
...
다음과 같이 파라미터명을 입력하지 않고 활용할 수 있습니다.
